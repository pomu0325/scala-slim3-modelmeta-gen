...
object MetaTemplate {
  ...
  val EntityToModelCiphered = """
    model.$$setter$$(decrypt(entity.getProperty("$$propname$$").asInstanceOf[$$typename$$]))"""
  ...
  object ModelToEntity {
    ...
    val Ciphered = """
    entity.setProperty("$$propname$$", encrypt(m.$$getter$$()))"""
  }
...
"""
  ...
  val IsCipherProperty = """
        if ("$$propname$$".equals(propertyName)) return true
"""
...
$$pre_put$$
  ...
  override def isCipherProperty(propertyName:String): Boolean = {
$$cipherprops$$
    false
  }
